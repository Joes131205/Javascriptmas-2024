...
function checkDuplicate() {
    ...
    const trimedText = itemText.trim().replace(/\s{2,}/g, " ");

    if (
        listArr.find((item) => item.toLowerCase() === trimedText.toLowerCase())
    ) {
        renderList();
...
function renderList() {
    ...
    listArr.forEach((gift) => {
        ...
        listItem.textContent = gift[0].toUpperCase() + gift.slice(1);
